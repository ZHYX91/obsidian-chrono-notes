...
export async function buildReleaseArchive({
  ...
  const resolvedOutputPath = path.resolve(
    outputPath ?? path.join(resolvedDistDirectory, `chrono-notes-${releaseVersion}.zip`),
  );
...
export async function compareExistingReleaseAssets({
  ...
  for (const assetName of assetNames) {
    ...
    if (localHash !== existingHash) {
      differences.push(
        `${assetName}: local ${localHash}, existing ${existingHash}`,
      );
    }
  }
...
async function main() {
  ...
  if (command === "compare") {
    ...
    process.stdout.write(
      `Existing Release ${version} has identical immutable assets (${assetNames.join(", ")}); publication is a no-op.\n`,
    );
    return;
  }
