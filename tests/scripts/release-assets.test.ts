...
describe("Release asset contract", () => {
  it("builds a byte-deterministic ZIP with fixed metadata and entry order", async () => {
    ...
    expect(archive.entries.map((entry) => entry.name)).toEqual([
      "chrono-notes/main.js",
      "chrono-notes/manifest.json",
      "chrono-notes/styles.css",
    ]);
    expect(
      archive.entries.map(({ content, crc32, ...metadata }) => metadata),
    ).toEqual([
      fixedEntryMetadata("chrono-notes/main.js"),
      fixedEntryMetadata("chrono-notes/manifest.json"),
      fixedEntryMetadata("chrono-notes/styles.css"),
    ]);
    expect(archive.entries.map((entry) => entry.content.toString("utf8"))).toEqual([
      "123456789",
      `${JSON.stringify({ id: "chrono-notes", version })}\n`,
      ".chrono-notes { color: red; }\n",
    ]);
  ...
  it("accepts an existing Release only when all four assets have identical hashes", async () => {
    ...
    await expect(
      ...
      stderr: expect.stringMatching(/manifest\.json[\s\S]*Existing Release asset manifest\.json is missing/u),
    });
  });
  ...
  it.each([
    ...
    "1.2.3-beta.1",
    "1.2.3+build.1",
  ])("rejects non-stable archive version %s before comparing the manifest", async (invalidVersion) => {
    const root = await createTemporaryDirectory();
    const distDirectory = path.join(root, "dist");
    await writeReleaseAssets(distDirectory);
    ...
    await expect(
      ...
      stderr: expect.stringContaining(
        "Release version must use stable x.y.z without a v prefix or leading zeroes",
      ),
    });
  });
});
...
async function createTemporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "chrono-release-assets-"));
