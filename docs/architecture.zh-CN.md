...
Properties 日期/时间显示与打开行为使用两个独立 Obsidian 适配器。显示适配器在主文档、公开 workspace API 枚举到的已有 leaf 文档及未来 `window-open` 提供的弹出文档中，用 MutationObserver 管理动态出现或替换的 `date` / `datetime-local` 输入；只有非系统且格式有效的非空值获得 `aria-hidden` 显示覆盖层。失焦时由 Obsidian 提供的 Moment 按独立日期/时间 pattern 格式化，系统侧的组合部分使用操作系统 `Intl`；聚焦、输入和编辑阶段隐藏覆盖层并露出原生输入。CSS 严格限定 `.metadata-properties`，覆盖层不接收指针事件；简单或可推断的年月日顺序用互斥根 class 保持聚焦后的原生字段顺序与选择器。切换设置、关窗及卸载会移除监听器、observer、覆盖层、输入/宿主 class 和根 class。该链路从不赋值 input、派发编辑事件、写 frontmatter 或访问 Vault；自定义格式 core 只允许一组本地年月日或时分秒/上下午 token、方括号字面量和转义，并拒绝 `Z / ZZ`。日期属性打开日记则由 pure ISO 日期输入解析器与捕获阶段适配器组成。打开适配器仅在设置开启、日记已配置、主键点击命中日期类型属性的打开图标且输入值有效时阻止原事件，并把日期与默认/新标签目标交给统一周期笔记命令。Properties 中的 Wiki/Markdown 链接、正文链接、非日期属性、非主键、未配置日记和非法日期全部透传。两个适配器互不依赖，也不建立第二份笔记状态。
