...
Properties 日期/时间显示与打开行为使用两个独立 Obsidian 适配器，并由一个 per-Document owner 管理同一组文档资源。它在主文档、公开 workspace API 枚举到的已有 leaf 文档及未来 `window-open` 提供的弹出文档中，同时挂载显示 observer 与捕获阶段点击 listener；`window-close`、卸载和异常清理从同一集合移除两者。owner 在首次挂载前即注册到运行时回滚集合，add、remove 与 dispose 均为幂等操作。显示适配器用 MutationObserver 管理动态出现或替换的 `date` / `datetime-local` 输入。只有非空、有效且至少一个生效组成部分为非系统格式的输入挂载 `aria-hidden` 装饰显示层；日期和时间均跟随系统时不挂载。HTML 日期控件的值是无时区 civil fields：Obsidian Moment 使用 UTC 模式读取字段，系统格式通过 `setUTCFullYear()` 与 UTC setters 构造只承载字段的 `Date` 并固定 `Intl timeZone: UTC`；这里的 UTC 不转换用户值，既保留 `0001`–`0099` 年，也阻止 DST 缺口或跳日把字段归一化到另一天/时刻。获得焦点或进入编辑时隐藏显示层并完整露出原生内部编辑器；失焦显示测得的行内尺寸在聚焦期间继续作用于 input，只稳定外部布局，不介入浏览器字段或选择器。CSS 严格限定于 `.metadata-properties`，显示层不接收指针事件；实现不得重排浏览器日期字段、隐藏原生选择器图标、伪造原生分隔符或强制原生编辑方向。适配器按 input 与可用行内空间测量显示几何，复制必要的输入文字度量与内距，并以逻辑方向定位，避免固定页面坐标且保留相邻 Obsidian 控件的正常布局；强制颜色下回退到完整原生控件。切换设置、关窗及卸载会移除监听器、observer、显示层及 input/host class。该链路从不赋值 input、派发编辑事件、写 frontmatter 或访问 Vault；自定义格式 core 只允许一组本地年月日或时分秒/上下午 token、方括号字面量和转义，并拒绝 `Z / ZZ`。日期属性打开日记则由 pure ISO 日期输入解析器与同一文档 owner 中的捕获阶段适配器组成。打开适配器仅在设置开启、日记已配置、主键点击命中日期类型属性的打开图标且输入值有效时阻止原事件，并把日期与默认/新标签目标交给统一周期笔记命令。Properties 中的 Wiki/Markdown 链接、正文链接、非日期属性、非主键、未配置日记和非法日期全部透传。两个适配器互不依赖，也不建立第二份笔记状态。
