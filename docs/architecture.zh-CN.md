...
设置结构带 `schemaVersion`；当前 schema v18 新增 `propertyDateDisplayFormat`、`propertyTimeDisplayFormat`、`propertyDateCustomFormat` 与 `propertyTimeCustomFormat`，旧配置迁移为日期/时间均 `system` 及合法的自定义默认 pattern。schema v17 在统一历法和周期路径契约时保留已发布 Chrono Notes 配置：新字段缺失时把 `calendarOverlays` 移入 `calendarExtensions` 并删除旧字段，把能够确认的 0.2.1 之前 Luxon 周期路径转换为文档化的 Obsidian/Moment 语法，同时保持已经合法的 Moment 格式和无法识别的自定义字符串，不用默认值覆盖。schema v16 把旧三档待办标注迁移为 `showTaskProgress: boolean`：`none` 迁移为 `false`，`color` 与 `hole` 迁移为 `true`，并删除旧字段。schema v15 使用 `confirmPeriodicNoteCreation` 与 `confirmIntervalNoteCreation` 分别表达周期笔记和区间笔记的创建确认，删除共享的 `confirmBeforeCreate`，不把旧值翻译为任一新开关。schema v14 使用 `showNoteIndicators: boolean` 表达可见状态，已删除 `indicatorPosition` 运行时字段和校验器，且不把旧位置值翻译为新开关；schema v10–v13 依次加入季度命名、字体模式/固定因子、任务标注和 Properties 日期接管。`calendarExtensions` 归一化只接受静态注册表中的 ID，保持首次出现顺序、去重并截取最多两项；设置 UI 用两个有序下拉槽位排除另一槽已选项。`holidayRegions` 同样只保留已知静态 provider ID、首次顺序与前三项，设置 UI 使用“第一个地区 / 第二个地区 / 第三个地区”三个有序槽位、排除重复并在清空前槽后自动前移。非法枚举回退，因子取整并夹在 `0–20`，布尔字段只接受真实布尔值。加载流程是“读取原始数据 → 按版本迁移 → 校验归一化 → 生成运行时设置”；最终归一化丢弃已删除字段。缺失、非法或早于 v1 的版本按 v1 处理，未来版本只复制原始数据而不由旧迁移器降级。未发布且使用不同插件 ID 的 My Calendar 扁平设置不属于自动迁移输入。保存设置后按字段差异触发定向 effect。

设置页保留两条明确的宿主路径：Obsidian 1.12 使用原有五页签 `display()`，1.13 使用同样五个页面的声明式 definitions 与设置搜索。1.13 的页面顺序固定为常规、外观与视图、周期笔记、区间笔记、扩展与集成，不通过数组重排冒充子页导航；两条路径都把插件解析后的 `ltr / rtl` 方向应用到设置容器。Obsidian 公共 API 不提供插件声明式子页深链，因此区间列表只承诺“打开 Chrono Notes 设置”，经宿主设置入口选择插件根页，不声称直接进入区间页。两条路径共享设置校验、持久化和副作用规则；声明式变化只有在页面结构或动态文字变化时调用完整 `update()`，只依赖 `visible` / `disabled` 谓词时使用 `refreshDomState()` 原位刷新。1.12 中每个持有外部资源的 section renderer 都返回幂等 cleanup；切页、重渲染和隐藏都必须先 cleanup、再清空 DOM。组合 cleanup 按挂载逆序释放，单项失败不能阻止其余资源释放，父层记录错误后仍继续下一次渲染。surface revision 会阻止异步保存续程在设置页隐藏后或原 section 已被替换后重新渲染；1.13 则先由基类 hide 生命周期释放声明式控件，再清空容器。自定义 `render` 输入只在实际挂载时启动 Vault 路径目录，其 cleanup 必须关闭建议器并立即刷新仍处于防抖窗口内的设置保存。

已有的旧 schema 设置完成迁移后，启动流程会在组装运行时前立即写回规范化的当前 schema 快照。写回失败只记录错误，不丢弃已经成功迁移的内存设置，也不阻止插件加载；之后任意一次成功的设置保存都会写入当前 schema。

`calendarExtensions` 是 schema v17 及以后唯一的历法选择字段。schema v17 仅在新字段缺失时把 `calendarOverlays` 作为一次性已发布设置来源；运行时归一化和所有消费者仍只读取新字段。

周期路径设置持久化 `pattern` 与对应模板文件路径，不引入派生文件夹字段。core 为路径和内置模板格式只接受文档列明的 Obsidian/Moment 子集，再编译到内部 Luxon 后端；运行时遇到不支持的 ASCII token 直接无效。schema v17 只在升级时转换文档化的旧 Luxon 日期 token 与单引号字面量，这是一次性数据迁移，不是第二套运行时解析器。`formatPeriodicNotePath()` 与 `parsePeriodicNotePath()` 继续构成唯一的格式化/反向识别契约，pure UI helper 从同一契约生成 `empty / invalid / valid` 冻结预览。Obsidian 原生输入建议只枚举 Vault 文件夹元数据或 `getMarkdownFiles()` 返回的路径：文件夹选择以 Moment 方括号字面量写回同一 pattern 并保留文件名格式。五类周期笔记各自在路径 pattern 旁写入自己的完整 Markdown 模板路径，区间笔记则把模板路径放在确定性创建目录旁；模板不能决定目标路径。“常规”按当前引擎说明内置占位符或注入的 `tp_calendar` helper，且不读取模板正文。这些 UI adapter 不解析笔记，也不形成 NoteIndex 之外的笔记状态来源。
...
历法渲染把主要日期表达与共享事件分层。日期表达 grid 对单个历法居中、对两个历法使用等宽双列，并且只在窄阈值下把该 grid 改为单列；位于其后的独立整格事件行始终相对完整日期格居中。视觉溢出可以折叠或省略事件文字，但不得从冻结模型删除事件。历法扩展子元素不设置原生 `title`；日期、周、月和季度格共用唯一的只读 preview portal，统一组合所有主要日期、去重事件的来源与交节时间、笔记/任务状态、节假日、ICS、受限正文摘要与嵌入统计，并且只读取所选冻结查询值。portal 把周期标题和嵌入统计保护在可收缩主区域之外，先缩减正文，极端情况下才折叠结构化信息的视觉区域，同时保持完整 ARIA。启用预览的控件同时设置 Obsidian 的 `--no-tooltip` 宿主保护；日历祖先容器、周概览、年热力图网格与日期格完整信息都由引用的视觉隐藏文本提供标签，不再使用直接 `aria-label`，从而在保留无障碍名称的同时避免第二个宿主提示。
