...
设置页保留两条明确的宿主路径：Obsidian 1.12 使用原有五页签 `display()`，1.13 使用同样五个页面的声明式 definitions 与设置搜索。两条路径共享设置校验、持久化和副作用规则；声明式变化只有在页面结构或动态文字变化时调用完整 `update()`，只依赖 `visible` / `disabled` 谓词时使用 `refreshDomState()` 原位刷新。自定义 `render` 输入只在实际挂载时启动 Vault 路径目录，其 cleanup 必须关闭建议器并立即刷新仍处于防抖窗口内的设置保存。
...
Properties 日期/时间显示与打开行为使用两个独立 Obsidian 适配器。显示适配器在主文档、公开 workspace API 枚举到的已有 leaf 文档及未来 `window-open` 提供的弹出文档中，用 MutationObserver 管理动态出现或替换的 `date` / `datetime-local` 输入。只有非空、有效且至少一个生效组成部分为非系统格式的输入挂载 `aria-hidden` 装饰显示层；日期和时间均跟随系统时不挂载。失焦时由 Obsidian 提供的 Moment 按独立日期/时间 pattern 格式化，组合值中跟随系统的部分继续使用操作系统 `Intl`。获得焦点或进入编辑时隐藏显示层并完整露出原生内部编辑器；失焦显示测得的行内尺寸在聚焦期间继续作用于 input，只稳定外部布局，不介入浏览器字段或选择器。CSS 严格限定于 `.metadata-properties`，显示层不接收指针事件；实现不得重排浏览器日期字段、隐藏原生选择器图标、伪造原生分隔符或强制原生编辑方向。适配器按 input 与可用行内空间测量显示几何，复制必要的输入文字度量与内距，并以逻辑方向定位，避免固定页面坐标且保留相邻 Obsidian 控件的正常布局；强制颜色下回退到完整原生控件。切换设置、关窗及卸载会移除监听器、observer、显示层及 input/host class。该链路从不赋值 input、派发编辑事件、写 frontmatter 或访问 Vault；自定义格式 core 只允许一组本地年月日或时分秒/上下午 token、方括号字面量和转义，并拒绝 `Z / ZZ`。日期属性打开日记则由 pure ISO 日期输入解析器与捕获阶段适配器组成。打开适配器仅在设置开启、日记已配置、主键点击命中日期类型属性的打开图标且输入值有效时阻止原事件，并把日期与默认/新标签目标交给统一周期笔记命令。Properties 中的 Wiki/Markdown 链接、正文链接、非日期属性、非主键、未配置日记和非法日期全部透传。两个适配器互不依赖，也不建立第二份笔记状态。
