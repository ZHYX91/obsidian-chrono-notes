...
历法渲染把主要日期表达与共享事件分层。日期表达 grid 对单个历法居中、对两个历法使用等宽双列，并且只在窄阈值下把该 grid 改为单列；位于其后的独立整格事件行始终相对完整日期格居中。视觉溢出可以折叠或省略事件文字，但不得从冻结模型删除事件。历法扩展子元素不设置原生 `title`；日期、周、月和季度格共用唯一的只读 preview portal，统一组合所有主要日期、去重事件的来源与交节时间、笔记/任务状态、节假日、ICS、受限正文摘要与嵌入统计，并且只读取所选冻结查询值。启用预览的控件同时设置 Obsidian 的 `--no-tooltip` 宿主保护；日历祖先容器与日期格完整信息都由引用的视觉隐藏文本提供标签，不再使用直接 `aria-label`，从而在保留无障碍名称的同时避免第二个宿主提示。
