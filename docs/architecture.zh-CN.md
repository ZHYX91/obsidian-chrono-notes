...
共享模板端口为周期笔记和区间笔记按设置显式选择内置引擎或 Templater，并在任何目标写入前解析完整 Vault 相对路径、确认 Markdown 模板存在并通过 `cachedRead()` 取得只读正文。内置周期上下文支持文档化的 `date/time/title` 占位符，区间上下文改用 `start/end/days/time/title`，两者都直接返回可用于单次创建的最终正文。Templater adapter 预检后返回延后渲染步骤，在目标文件创建后注入按类型冻结的 `tp_calendar` 上下文并返回渲染正文。Templater 不可用或执行失败时抛出明确错误，不允许静默回退到内置引擎。区间模板正文由 core 在写入前强制合并规范 `start`/`end` frontmatter，同时保留其他有效 mapping 属性和正文；模板 frontmatter 非法或未闭合时创建失败，且内置路径不会留下需要回滚的目标文件。
