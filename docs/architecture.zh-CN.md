...
历法渲染把主要日期表达与共享事件分层。日期表达 grid 对单个历法居中、对两个历法使用等宽双列，并且只在窄阈值下把该 grid 改为单列；位于其后的独立整格事件行始终相对完整日期格居中。视觉溢出可以折叠或省略事件文字，但不得从冻结模型删除事件。历法扩展子元素不设置原生 `title`；日期格唯一的 preview portal 统一组合所有主要日期、去重事件的来源与交节时间、笔记/任务状态、节假日和 ICS。同一完整信息由视觉隐藏文本通过 `aria-labelledby` 提供，避免 Obsidian 把直接 `aria-label` 转成宿主提示；关闭悬停预览后也不存在扩展子元素 tooltip 旁路。
