...
运行时 i18n 位于 shared 层，不依赖 Obsidian 或 React。简体中文目录是类型安全 message key 的权威来源；英文和繁体中文使用完整 `Record`，阿拉伯语、波斯语、希伯来语、阿姆哈拉语和印地语则通过与英文 key 顺序绑定的紧凑完整目录构造器生成，构建与测试同时固定键全集和逐消息插值占位符，不接受依赖缺失消息回退的 partial catalog。`auto` 根据系统 locale 的中文脚本/地区区分简繁，并识别五种新增语言，其他未知语言回退英文。translator 负责参数插值、基于 `Intl.PluralRules` 的计数形式和根方向；阿拉伯语、波斯语与希伯来语输出 `rtl`，其余为 `ltr`，设置页与 React 日历根节点同步应用。日期、月份和数字格式继续交给 `Intl`，CSS 使用逻辑方向属性。日期格顶部 accessory 行是有意的物理方向例外：笔记状态或任务进度始终位于物理左上，地区标记始终位于物理右上，但标记自身文字方向仍跟随 translator。组件不得新增分散的 locale 分支或自行维护消息表。
